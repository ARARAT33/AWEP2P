...
pub fn save_json<T: Serialize>(path: &Path, value: &T, passphrase: &str) -> Result<(), SecureStateError> {
    if passphrase.is_empty() { return Err(SecureStateError::EmptyPassphrase); }
    ...
    let password_hash = Argon2::default().hash_password(passphrase.as_bytes(), &salt_string)
    ...
    let ciphertext = cipher.encrypt(Nonce::from_slice(&nonce), plaintext.as_ref()).map_err(|_| SecureStateError::Crypto)?;
...
pub fn load_json<T: DeserializeOwned>(path: &Path, passphrase: &str) -> Result<T, SecureStateError> {
    if passphrase.is_empty() { return Err(SecureStateError::EmptyPassphrase); }
    ...
    let plaintext = cipher.decrypt(Nonce::from_slice(nonce), &envelope[nonce_start + NONCE_LEN..])
        .map_err(|_| SecureStateError::Crypto)?;
