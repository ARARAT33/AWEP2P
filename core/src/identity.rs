...
impl AweId { pub fn from_public_key(public_key: &[u8]) -> Self { Self(hash(b"AWE-ID/v1", public_key)) } pub fn as_bytes(&self) -> &[u8; 32] { &self.0 } pub fn to_hex(&self) -> String { hex::encode(self.0) } }

#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
pub struct Username(String);
impl Username {
    pub fn new(value: impl Into<String>) -> Result<Self, &'static str> { let value=value.into(); if value.is_empty() || value.len()>64 || !value.chars().all(|c| c.is_ascii_alphanumeric() || c=='_' || c=='-') { return Err("invalid username"); } Ok(Self(value)) }
...
impl Identity {
    pub fn generate(username: Username) -> Self { Self::from_signing_key(username, SigningKey::generate(&mut OsRng)) }
    fn from_signing_key(username: Username, signing_key: SigningKey) -> Self { let public_key=signing_key.verifying_key().to_bytes(); let awe_id=AweId::from_public_key(&public_key); Self { signing_key, public: IdentityPublic { username, awe_id, public_key } } }
    pub fn sign(&self, message: &[u8]) -> [u8;64] { self.signing_key.sign(message).to_bytes() }
    pub fn verify(public_key: &[u8;32], message: &[u8], signature: &[u8;64]) -> bool { VerifyingKey::from_bytes(public_key).ok().and_then(|k| k.verify(message, &Signature::from_bytes(signature)).ok()).is_some() }
...
impl LocalVault {
    pub fn seal(identity:&Identity, password:&str)->Result<Vec<u8>,VaultError>{
        ...
        serde_json::to_vec(&VaultRecord{version:VAULT_VERSION,password_hash,nonce:nonce_bytes,ciphertext}).map_err(|_|VaultError::InvalidFormat)
    }
    pub fn open(data:&[u8],username:Username,password:&str)->Result<Identity,VaultError>{
        ...
        let key_material=parsed.hash.ok_or(VaultError::InvalidFormat)?; let key=Sha256::digest(key_material.as_bytes());
        let cipher=ChaCha20Poly1305::new_from_slice(&key).map_err(|_|VaultError::Encryption)?;
        ...
        let mut bytes=[0u8;32]; bytes.copy_from_slice(&secret); secret.zeroize(); Ok(Identity::from_secret(username,bytes))
    }
}

pub fn secure_random<const N:usize>() -> Zeroizing<[u8;N]> { let mut bytes=Zeroizing::new([0u8;N]); OsRng.fill_bytes(&mut *bytes); bytes }
...
mod tests { use super::*;
    ...
    #[test] fn random_is_nonconstant(){assert_ne!(secure_random::<32>().as_ref(),secure_random::<32>().as_ref());}
}
