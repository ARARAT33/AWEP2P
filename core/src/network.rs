...
fn encode(value: &Control) -> Result<Vec<u8>, NetworkError> {
    serde_json::to_vec(value).map_err(|e| NetworkError::Protocol(e.to_string()))
}

fn decode(value: &[u8]) -> Result<Control, NetworkError> {
    serde_json::from_slice(value).map_err(|e| NetworkError::Protocol(e.to_string()))
}
...
async fn handshake(
    ...
    let local_signature = identity.sign(&hello_bytes(
        PROTOCOL_VERSION,
        &local_id,
        &local_public_key,
        &ephemeral_public,
    ...
    let remote = if initiator {
        ...
        decode(&timeout(HELLO_TIMEOUT, read_raw(&mut stream))
            .await
            .map_err(|_| NetworkError::Timeout)??)?
    } else {
        let remote = decode(&timeout(HELLO_TIMEOUT, read_raw(&mut stream))
            .await
            .map_err(|_| NetworkError::Timeout)??)?;
        write_raw(&mut stream, &encode(&local_hello)?).await?;
        remote
    ...
    let (remote_id, remote_public_key, remote_ephemeral, remote_nonce, remote_signature, version) =
        match remote {
            Control::Hello {
                version,
                awe_id,
    ...
    let (low_id, high_id, low_nonce, high_nonce) = if local_id < remote_id {
    ...
    hkdf.expand(b"AWE/SESSION/v1", &mut key_material)
        .map_err(|_| NetworkError::Encryption)?;

    let (tx_key, rx_key) = if initiator {
        (&key_material[..32], &key_material[32..])
    } else {
        (&key_material[32..], &key_material[..32])
    };
    ...
    Ok(SecureConnection {
        ...
        remote_id,
        remote_public_key,
        tx: ChaCha20Poly1305::new_from_slice(tx_key).map_err(|_| NetworkError::Encryption)?,
        rx: ChaCha20Poly1305::new_from_slice(rx_key).map_err(|_| NetworkError::Encryption)?,
        tx_seq: 0,
...
impl SecureConnection {
    ...
    async fn send(&mut self, control: &Control) -> Result<(), NetworkError> {
        ...
        let ciphertext = self
            .tx
            .encrypt(
                Nonce::from_slice(&Self::nonce(sequence)),
                Payload {
                    msg: &plaintext,
                    aad: &aad,
                },
    ...
    async fn recv(&mut self) -> Result<Control, NetworkError> {
        ...
        if !self.replay.accept(self.remote_id, sequence) {
...
impl RoutingTable {
    pub fn insert(&mut self, peer: PeerRecord) {
        self.peers.insert(peer.awe_id, peer);
    }
    ...
    pub fn closest(&self, target: &[u8; 32], limit: usize) -> Vec<PeerRecord> {
        ...
        peers.sort_by_key(|peer| xor_distance(&peer.awe_id, target));
...
fn xor_distance(a: &[u8; 32], b: &[u8; 32]) -> [u8; 32] {
    ...
    for index in 0..32 {
        distance[index] = a[index] ^ b[index];
    }
    distance
}
...
impl Node {
    ...
    async fn handle_connection(
        ...
        let Ok(mut connection) = handshake(stream, identity, false).await else {
            return;
        };
        let record = PeerRecord {
            awe_id: connection.remote_id,
            public_key: connection.remote_public_key,
        ...
        loop {
            match timeout(HEARTBEAT_INTERVAL, connection.recv()).await {
                Ok(Ok(Control::Ping { sequence })) => {
                    if connection.send(&Control::Pong { sequence }).await.is_err() {
                        break;
                    }
                }
                Ok(Ok(Control::Pong { .. })) => {}
                Ok(Ok(Control::FindNode { target })) => {
                    ...
                    if connection.send(&Control::Nodes { records }).await.is_err() {
                        break;
                    }
                }
                Ok(Ok(Control::Data { .. })) => {}
                Ok(Ok(Control::Nodes { .. })) | Ok(Ok(Control::Hello { .. })) => break,
                Ok(Err(_)) => break,
                Err(_) => {
                    if connection.is_idle() {
                        break;
                    }
                    if connection.ping(heartbeat_sequence).await.is_err() {
                        break;
                    }
    ...
    pub async fn listen(&self) -> Result<(), NetworkError> {
        ...
        loop {
            ...
            tokio::spawn(Self::handle_connection(
                stream, address, identity, routing, peers,
            ));
        }
    }

    pub async fn connect(&self, address: SocketAddr) -> Result<SecureConnection, NetworkError> {
        let stream = timeout(HELLO_TIMEOUT, TcpStream::connect(address))
            .await
            .map_err(|_| NetworkError::Timeout)??;
...
mod tests {
    ...
    #[tokio::test]
    async fn node_bootstrap_discovers_peer() {
        let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
        let address = listener.local_addr().unwrap();
        let server_identity = Identity::generate(Username::new("server").unwrap());
        let client_identity = Identity::generate(Username::new("client").unwrap());
        let routing = Arc::new(RwLock::new(RoutingTable::default()));
